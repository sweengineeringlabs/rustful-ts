...
impl GridSearch {
    ...
    pub fn optimize_arima(
        ...
        data: &[f64],
        horizon: usize,
    ) -> algorithm::Result<(usize, usize, usize, f64)> {
    ...
    /// Returns (alpha, score)
    pub fn optimize_ses(&self, data: &[f64], horizon: usize) -> algorithm::Result<(f64, f64)> {
        let (train, test) = train_test_split(data, self.test_ratio);
        let test_len = test.len().min(horizon);
    ...
    pub fn optimize_holt(&self, data: &[f64], horizon: usize) -> algorithm::Result<(f64, f64, f64)> {
        let (train, test) = train_test_split(data, self.test_ratio);
        let test_len = test.len().min(horizon);
    ...
    pub fn optimize_knn(
        ...
        data: &[f64],
        horizon: usize,
    ) -> algorithm::Result<(usize, usize, f64)> {
        let (train, test) = train_test_split(data, self.test_ratio);
        let test_len = test.len().min(horizon);
        ...
        for &k in &k_values {
            for &window in &window_values {
                if let Ok(model) =
                    TimeSeriesKNN::new(k, window, DistanceMetric::Euclidean)
                {
...
mod tests {
    ...
    fn sample_data() -> Vec<f64> {
        ...
        (0..50).map(|i| 10.0 + i as f64 * 0.5 + (i as f64 * 0.3).sin()).collect()
    }
