...
///
/// ```rust
/// use predictor_api::smoothing::SimpleExponentialSmoothing;
...
impl SimpleExponentialSmoothing {
    ...
    pub fn auto(data: &[f64]) -> Result<Self> {
        ...
        model
            .fit(data)
            .map_err(|e| TsError::InvalidData(e.to_string()))?;
        Ok(model)
    }
...
///
/// ```rust
/// use predictor_api::smoothing::DoubleExponentialSmoothing;
/// use predictor_api::Predictor;
///
...
///
/// ```rust
/// use predictor_api::smoothing::{HoltWinters, SeasonalType};
/// use predictor_api::Predictor;
///
