...
//!
//! ```rust,no_run
//! use data_core::{fetch_stock, YahooFinance};
...
use data_spi::{DataError, DataSource, Interval, Quote, Result};
...
impl YahooFinance {
    ...
    fn parse_date(date: &str) -> Result<i64> {
        ...
        if parts.len() != 3 {
            return Err(DataError::InvalidDate(format!(
        ...
        let year: i32 = parts[0]
            .parse()
            .map_err(|_| DataError::InvalidDate(format!("Invalid year: {}", parts[0])))?;
        let month: i32 = parts[1]
            .parse()
            .map_err(|_| DataError::InvalidDate(format!("Invalid month: {}", parts[1])))?;
        let day: i32 = parts[2]
            .parse()
            .map_err(|_| DataError::InvalidDate(format!("Invalid day: {}", parts[2])))?;
    ...
    fn parse_response(&self, json: &str) -> Result<Vec<Quote>> {
        ...
        let data = results.first().ok_or(DataError::NoData)?;

        let quote_data = data.indicators.quote.first().ok_or(DataError::NoData)?;
...
impl DataSource for YahooFinance {
    ...
    fn fetch_sync(
        ...
        #[cfg(feature = "fetch")]
        {
            self.fetch_blocking(symbol, start_date, end_date, interval)
        }
        #[cfg(not(feature = "fetch"))]
        {
            let _ = (symbol, start_date, end_date, interval);
            Err(DataError::ConfigError(
                "fetch feature not enabled".to_string(),
            ))
        }
    }
}
...
// ============================================================================
// Tests (private method tests must stay here)
// ============================================================================
...
mod tests {
    ...
    fn test_parse_response_invalid_json() {
        let client = YahooFinance::new();
        assert!(matches!(client.parse_response("not json"), Err(DataError::ParseError(_))));
    }
