//! Crab Harmonic Pattern implementation.
//!
//! The Crab pattern is a harmonic pattern that features the most extended
//! CD leg among the standard harmonic patterns. It uses Fibonacci ratios
//! to identify potential reversal zones at extreme price extensions.
//!
//! Ideal Fibonacci ratios:
//! - B retraces 0.382-0.618 of XA
//! - AB leg is 0.382-0.886 of XA
//! - BC leg is 0.382-0.886 of AB
//! - CD leg extends 2.618-3.618 of BC (the defining characteristic)
...
/// Configuration for Crab pattern detection.
#[derive(Debug, Clone)]
pub struct CrabPatternConfig {
...
impl Default for CrabPatternConfig {
    fn default() -> Self {
        Self {
            lookback: 100,
            tolerance: 0.05,
...
/// Output from Crab pattern detection.
#[derive(Debug, Clone)]
pub struct CrabPatternOutput {
...
/// Crab Harmonic Pattern indicator.
///
/// Identifies bullish and bearish Crab patterns using XABCD price legs with
/// Fibonacci ratio validation. The Crab pattern is distinguished by its
/// extreme CD leg extension (2.618-3.618 of BC), which produces the most
/// extended potential reversal zone of the standard harmonic patterns.
///
/// - Bullish Crab: X(low) -> A(high) -> B(low) -> C(high) -> D(low)
/// - Bearish Crab: X(high) -> A(low) -> B(high) -> C(low) -> D(high)
#[derive(Debug, Clone)]
...
impl CrabPattern {
    pub fn new(config: CrabPatternConfig) -> Self {
        Self {
    ...
    fn score_pattern(&self, ab_xa: f64, bc_ab: f64, cd_bc: f64) -> f64 {
        ...
        let cd_score = if self.ratio_in_range(cd_bc, cd_bc_range.0, cd_bc_range.1) {
            // The CD extension is the defining characteristic of the Crab;
            // give full marks when in range, with a slight bonus for being
            // near the center (3.118).
            let center = (cd_bc_range.0 + cd_bc_range.1) / 2.0;
            let range_half = (cd_bc_range.1 - cd_bc_range.0) / 2.0;
            let dist_from_center = (cd_bc - center).abs();
            33.4 * (1.0 - (dist_from_center / range_half).min(1.0) * 0.2)
        } else {
    ...
    /// Detect Crab patterns in the OHLCV data.
    pub fn calculate(&self, high: &[f64], low: &[f64]) -> CrabPatternOutput {
        let n = high.len();
        ...
        for w in 0..points.len().saturating_sub(4) {
            ...
            if !x.is_high && a.is_high && !b.is_high && c.is_high && !d.is_high {
                ...
                // Validate Crab ratios:
                // XA retracement 0.382-0.618, BC 0.382-0.886 of AB, CD 2.618-3.618 of BC
                if self.ratio_in_range(ab_xa_ratio, 0.382, 0.618)
                    && self.ratio_in_range(bc_ab_ratio, 0.382, 0.886)
                    && self.ratio_in_range(cd_bc_ratio, 2.618, 3.618)
                {
...
impl Default for CrabPattern {
    fn default() -> Self {
        Self::new(CrabPatternConfig::default())
    }
}

impl TechnicalIndicator for CrabPattern {
    fn name(&self) -> &str {
        "CrabPattern"
    }
...
impl SignalIndicator for CrabPattern {
...
mod tests {
    ...
    /// Generate synthetic data with an embedded bullish Crab-like structure.
    fn generate_crab_data() -> OHLCVSeries {
        // Create a price series with swing points that approximate Crab ratios:
        // X(low=90) -> A(high=110) -> B(low ~100, 0.50 retrace of XA=20)
        // -> C(high ~104) -> D(low ~92, CD extends ~3.0 of BC=4)
        let prices: Vec<f64> = vec![
            100.0, 97.0, 94.0, 92.0, 90.0, // decline to X
            93.0, 97.0, 102.0, 107.0, 110.0, // rally to A
            109.0, 107.0, 104.0, 102.0, 100.0, // retrace to B (~0.50 of XA)
            101.0, 102.0, 103.0, 103.5, 104.0, // rally to C
            102.0, 99.0, 96.0, 94.0, 92.0, // decline to D (CD ~3.0 of BC)
            93.0, 95.0, 97.0, 99.0, 101.0, // subsequent recovery
        ];
    ...
    #[test]
    fn test_crab_basic_compute() {
        let pattern = CrabPattern::default();
        let data = generate_crab_data();
    ...
    #[test]
    fn test_crab_insufficient_data() {
        let pattern = CrabPattern::default();
    ...
    #[test]
    fn test_crab_config_defaults() {
        let config = CrabPatternConfig::default();
        assert_eq!(config.lookback, 100);
        assert!((config.tolerance - 0.05).abs() < 1e-10);
    }

    #[test]
    fn test_crab_signal_generation() {
        let pattern = CrabPattern::default();
        let data = generate_crab_data();
    ...
    #[test]
    fn test_crab_cd_extension_range() {
        let pattern = CrabPattern::new(CrabPatternConfig {
            lookback: 100,
            tolerance: 0.05,
        });

        // Crab CD extension range: 2.618-3.618
        assert!(pattern.ratio_in_range(3.0, 2.618, 3.618));
        assert!(pattern.ratio_in_range(2.618, 2.618, 3.618));
        assert!(pattern.ratio_in_range(3.618, 2.618, 3.618));
        // With tolerance
        assert!(pattern.ratio_in_range(2.58, 2.618, 3.618)); // 2.618 - 0.05 = 2.568
        // Well outside
        assert!(!pattern.ratio_in_range(1.5, 2.618, 3.618));
        assert!(!pattern.ratio_in_range(4.0, 2.618, 3.618));

        // XA retracement range: 0.382-0.618
        assert!(pattern.ratio_in_range(0.50, 0.382, 0.618));
        assert!(!pattern.ratio_in_range(0.80, 0.382, 0.618));
    }
}
