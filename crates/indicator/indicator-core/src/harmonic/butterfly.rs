//! Butterfly Harmonic Pattern implementation.
//!
//! The Butterfly is a reversal pattern defined by specific Fibonacci ratios
//! between the XABCD legs. It extends beyond the initial XA leg, making it
//! useful for identifying potential reversal zones at extreme price levels.
//!
...
/// Configuration for Butterfly pattern detection.
#[derive(Debug, Clone)]
pub struct ButterflyPatternConfig {
...
impl Default for ButterflyPatternConfig {
    fn default() -> Self {
        Self {
            lookback: 100,
            tolerance: 0.05,
...
/// Output from Butterfly pattern detection.
#[derive(Debug, Clone)]
pub struct ButterflyPatternOutput {
...
/// Butterfly Harmonic Pattern indicator.
///
/// Identifies bullish and bearish Butterfly patterns using XABCD price legs
/// with Fibonacci ratio validation. The Butterfly pattern is characterized by
/// a B point retracement of 0.786 of the XA leg, with the CD leg extending
/// 1.618-2.618 of BC.
///
/// - Bullish Butterfly: X(low) -> A(high) -> B(low) -> C(high) -> D(low)
/// - Bearish Butterfly: X(high) -> A(low) -> B(high) -> C(low) -> D(high)
#[derive(Debug, Clone)]
...
impl ButterflyPattern {
    pub fn new(config: ButterflyPatternConfig) -> Self {
        Self {
    ...
    /// Check if a ratio is within tolerance of a target value.
    fn ratio_matches(&self, actual: f64, target: f64) -> bool {
        (actual - target).abs() <= self.tolerance
    }
    ...
    /// Calculate how well ratios match the ideal Butterfly pattern.
    /// Returns a confidence score from 0 to 100.
    fn score_pattern(&self, ab_xa: f64, bc_ab: f64, cd_bc: f64) -> f64 {
        // Ideal Butterfly ratios
        let xa_retrace_target = 0.786;
        let ab_xa_range = (0.382, 0.886);
        let bc_ab_range = (0.382, 0.886);
        let cd_bc_range = (1.618, 2.618);

        // Score each ratio (0-25 points each, total 100)
        let xa_score = if self.ratio_matches(ab_xa, xa_retrace_target) {
            25.0
        } else {
            let dist = (ab_xa - xa_retrace_target).abs();
            (25.0 - (dist / self.tolerance) * 12.5).max(0.0)
        };

        let ab_score = if self.ratio_in_range(ab_xa, ab_xa_range.0, ab_xa_range.1) {
            25.0
        } else {
            let dist_low = (ab_xa - ab_xa_range.0).abs();
            let dist_high = (ab_xa - ab_xa_range.1).abs();
        ...
        xa_score + ab_score + bc_score + cd_score
    }

    /// Detect Butterfly patterns in the OHLCV data.
    pub fn calculate(&self, high: &[f64], low: &[f64]) -> ButterflyPatternOutput {
        let n = high.len();
        ...
        for w in 0..points.len().saturating_sub(4) {
            ...
            if !x.is_high && a.is_high && !b.is_high && c.is_high && !d.is_high {
                ...
                if self.ratio_matches(ab_xa_ratio, 0.786)
...
impl Default for ButterflyPattern {
    fn default() -> Self {
        Self::new(ButterflyPatternConfig::default())
    }
}

impl TechnicalIndicator for ButterflyPattern {
    fn name(&self) -> &str {
        "ButterflyPattern"
    }
    ...
    fn min_periods(&self) -> usize {
        // Need at least enough bars for swing detection and pattern formation
...
impl SignalIndicator for ButterflyPattern {
...
mod tests {
    ...
    /// Generate synthetic data with an embedded bullish Butterfly-like structure.
    fn generate_butterfly_data() -> OHLCVSeries {
        // Create a price series with swing points that approximate Butterfly ratios:
        // X(low=90) -> A(high=110) -> B(low ~94.28, 0.786 retrace) -> C(high ~104) -> D(low ~88)
        let prices: Vec<f64> = vec![
            100.0, 98.0, 95.0, 92.0, 90.0, // decline to X
            93.0, 97.0, 102.0, 107.0, 110.0, // rally to A
            108.0, 105.0, 100.0, 97.0, 94.3, // retrace to B (0.786 of XA=20 -> B at 110-15.72=94.28)
            96.0, 99.0, 101.0, 103.0, 104.0, // rally to C
            102.0, 99.0, 96.0, 93.0, 88.0, // decline to D (CD extends 1.618+ of BC)
            89.0, 91.0, 93.0, 95.0, 97.0, // subsequent recovery
        ];
    ...
    #[test]
    fn test_butterfly_basic_compute() {
        let pattern = ButterflyPattern::default();
        let data = generate_butterfly_data();
    ...
    #[test]
    fn test_butterfly_insufficient_data() {
        let pattern = ButterflyPattern::default();
    ...
    #[test]
    fn test_butterfly_config_defaults() {
        let config = ButterflyPatternConfig::default();
        assert_eq!(config.lookback, 100);
        assert!((config.tolerance - 0.05).abs() < 1e-10);
    }

    #[test]
    fn test_butterfly_signal_generation() {
        let pattern = ButterflyPattern::default();
        let data = generate_butterfly_data();
    ...
    #[test]
    fn test_butterfly_ratio_matching() {
        let pattern = ButterflyPattern::new(ButterflyPatternConfig {
            lookback: 100,
            tolerance: 0.05,
        });

        // Exact match
        assert!(pattern.ratio_matches(0.786, 0.786));
        // Within tolerance
        assert!(pattern.ratio_matches(0.79, 0.786));
        // Outside tolerance
        assert!(!pattern.ratio_matches(0.9, 0.786));

        // Range matching
        assert!(pattern.ratio_in_range(0.5, 0.382, 0.886));
        assert!(!pattern.ratio_in_range(0.2, 0.382, 0.886));
    }
}
