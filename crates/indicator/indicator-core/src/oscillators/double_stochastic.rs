...
impl DoubleStochastic {
    ...
    fn stochastic(high: &[f64], low: &[f64], close: &[f64], period: usize) -> Vec<f64> {
        ...
        for i in (period - 1)..n {
            ...
            if range == 0.0 {
                ...
                result.push((((close[i] - lowest) / range) * 100.0).clamp(0.0, 100.0));
            }
        }
    ...
    fn stochastic_of_values(data: &[f64], period: usize) -> Vec<f64> {
        ...
        for i in (period - 1)..n {
            let start_idx = i + 1 - period;
            let window: Vec<f64> = data[start_idx..=i]
                .iter()
                .filter(|x| !x.is_nan())
                .copied()
                .collect();
            ...
            if current.is_nan() {
                ...
                // Clamp to [0, 100] to handle floating point precision issues
                result.push((((current - lowest) / (highest - lowest)) * 100.0).clamp(0.0, 100.0));
            }
        }
    ...
    pub fn calculate(&self, high: &[f64], low: &[f64], close: &[f64]) -> (Vec<f64>, Vec<f64>) {
        ...
        // Smooth %K
        let k_raw = Self::sma(&stoch2, self.smooth_k);
        ...
        // %D is SMA of %K
        let d_raw = Self::sma(&k, self.smooth_d);
...
mod tests {
    ...
    fn test_double_stoch_range() {
        ...
        // Double Stochastic should be in range [0, 100] (with small tolerance for floating point)
        for val in k.iter().chain(d.iter()) {
            if !val.is_nan() {
                assert!(*val >= -1e-10 && *val <= 100.0 + 1e-10, "Value {} out of range", val);
            }
        }
    }
}
