...
impl IntradayMomentumIndex {
    ...
    pub fn calculate(&self, open: &[f64], close: &[f64]) -> Vec<f64> {
        ...
        for i in (self.period - 1)..n {
            ...
            let sum_gains: f64 = gains[start_idx..=i].iter().sum();
            let sum_losses: f64 = losses[start_idx..=i].iter().sum();
            let total = sum_gains + sum_losses;

            if total == 0.0 {
                result.push(50.0);
            } else {
                let value = (sum_gains / total) * 100.0;
                // Clamp to [0, 100] to handle floating point precision issues
                result.push(value.clamp(0.0, 100.0));
            }
        }
