...
impl Decycler {
    ...
    pub fn calculate(&self, data: &[f64]) -> (Vec<f64>, Vec<f64>) {
        ...
        let cos_a = angle.cos();
        let sin_a = angle.sin();
        let alpha = ((cos_a + sin_a - 1.0) / cos_a).max(0.0).min(1.0);
...
mod tests {
    ...
    fn test_decycler_removes_cycles() {
        ...
        // Decycler should have lower std dev than input
        // Note: with period=30, some cycle energy remains
        assert!(dec_std < 10.0,
            "Decycler std dev {} should be less than input std dev (~10.6)", dec_std);
    }
    ...
    fn test_decycler_period_effect() {
        ...
        // Longer period = higher cutoff frequency = more cycles removed = lower variance
        // Shorter period = lower cutoff frequency = fewer cycles removed = higher variance
        assert!(long_var < short_var,
            "Long period variance ({}) should be < short period variance ({}) since longer period removes more cycles",
            long_var, short_var);
    }
