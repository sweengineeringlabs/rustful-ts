...
mod tests {
    ...
    fn test_laguerre_rsi_gamma_effect() {
        ...
        let n = 50;
        // Create data with continuous price changes to test responsiveness
        let data: Vec<f64> = (0..n).map(|i| {
            100.0 + (i as f64 * 0.3).sin() * 10.0
        }).collect();

        let rsi_fast = lrsi_fast.calculate(&data);
        let rsi_slow = lrsi_slow.calculate(&data);

        // Fast RSI should have more variance (reacts more to price changes)
        // Calculate variance of each RSI
        let fast_mean: f64 = rsi_fast[20..n].iter().sum::<f64>() / (n - 20) as f64;
        let slow_mean: f64 = rsi_slow[20..n].iter().sum::<f64>() / (n - 20) as f64;

        let fast_var: f64 = rsi_fast[20..n].iter()
            .map(|x| (x - fast_mean).powi(2))
            .sum::<f64>() / (n - 20) as f64;
        let slow_var: f64 = rsi_slow[20..n].iter()
            .map(|x| (x - slow_mean).powi(2))
            .sum::<f64>() / (n - 20) as f64;

        assert!(fast_var > slow_var,
            "Fast RSI variance ({}) should be > slow RSI variance ({}) since it's more responsive",
            fast_var, slow_var);
    }
